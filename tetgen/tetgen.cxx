...
bool tetgenio::load_medit(char* filename)
{
  ...
  // Medit requires the index starts from '1'.
  firstnumber = 1;
...
///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// load_gid()    Load a surface mesh described in .msh file.                 //
//                                                                           //
// 'filename' is the file name with extension .msh or without entension (The //
// .msh will be added in this case). .msh is the file format of GID, a user- //
// friendly interactive program.                                             //
//                                                                           //
// This routine ONLY reads the sections containing vertices and triangles,   //
// other sections (such as tetrahedra, edges, ...) are ignored.              //
//                                                                           //
...
bool tetgenio::load_gid(char* filename)
{ 
  return true;
}
...
bool tetgenio::load_plc(char* filename, int object)
{
  ...
  case tetgenbehavior::GID:
    return load_gid(filename);
  default:
    return load_poly(filename);
  }
}
...
{
  printf("Version 1.3.1 (Released on September 13, 2004).\n");
}
...
{
  printf("  tetgen [-pq__a__Ars__iMT__dzo_fengGOBNEFICQVvh] input_file\n");
...
void tetgenbehavior::usage()
{
  ...
  versioninfo();
  printf("\n");
  printf("Copyright 2002, 2004 Hang Si\n");
  ...
  printf("switch\n");
  printf("  and its numeric parameter.  input_file must be a file with ");
  printf("extension\n");
  printf("  .node, or .poly, or .smesh, or .ele depending on the switches ");
  printf("you\n");
  printf("  supplied.  File formats and detailed description of command ");
  printf("line\n");
  printf("  switches are found in user's manual.\n");
  printf("\n");
  syntax();
  printf("\n");
...
bool tetgenbehavior::parse_commandline(int argc, char **argv)
{
  ...
  if (startindex == 0) {
    ...
    if (!strcmp(&infilename[strlen(infilename) - 5], ".node")) {
      ...
      object = PLY;
      plc = 1;
    } else if (!strcmp(&infilename[strlen(infilename) - 4], ".msh")) {
      infilename[strlen(infilename) - 4] = '\0';
      object = GID;
      plc = 1;
  ...
  if (detectinter && (quality || insertaddpoints || (order == 2) || neighbors
      || checkclosure || docheck)) {
    printf("Error:  Switches %s cannot use together with -d.\n",
           "-c, -q, -i, -o2, -n, and -C");
    return false;
  }
  if (checkclosure && (quality || insertaddpoints || (order == 2) || neighbors
      || detectinter || docheck)) {
...
inline tetgenmesh::badface* tetgenmesh::sub2badface(face& s) {
  return (badface*) s.sh[11];
}

inline void tetgenmesh::setsub2badface(face& s, badface* value) {
  s.sh[11] = (shellface) value;
}

// These primitives are only suitable for subsegments which read or set the
//   type of a subsegment (i.e., SHARPSEGMNET or NONSHARPSEGMENT);  The type
//   value is stored in the first segment field sh[8].

inline enum tetgenmesh::subsegtype tetgenmesh::segmenttype(face& s) {
  return (enum subsegtype) ((unsigned long) s.sh[8]);
}

inline void tetgenmesh::setsegmenttype(face& s, enum subsegtype value) {
  s.sh[8] = (shellface) value;
} 

// These primitives read or set a shell marker.  Shell markers are used
//   to hold user boundary information.

inline int tetgenmesh::mark(face& s) { 
  return (* (int *) (s.sh + 12)); 
}

inline void tetgenmesh::setmark(face& s, int value) {
  * (int *) (s.sh + 12) = value;
}
...
inline enum tetgenmesh::vertextype tetgenmesh::pointtype(point pt) {
  return (enum vertextype) ((int *) (pt))[pointmarkindex + 1];
}

inline void tetgenmesh::setpointtype(point pt, enum vertextype value) {
...
// These two primitives set a pointer to another point (it's parent) to
//   current point.  They use the same field as above two primitives.
...
void tetgenmesh::findface(triface *fface, point forg, point fdest, point fapex)
{
  ...
  if (isdead(fface) || (org(*fface) != forg) || (dest(*fface) != fdest) ||
    ...
    enum vertextype forgty, fdestty, fapexty;
    int share, i;
...
void tetgenmesh::printtet(triface* tface)
{
  ...
  if (b->useshelles) {
    ...
    while(facecount < 4) {
      ...
      if(tmpsh.sh != dummysh) {
        printf("      [%i] x%lx  ID(%i).\n", facecount,
               (unsigned long)(tmpsh.sh), mark(tmpsh));
      }
      facecount ++;
    }
  }
...
void tetgenmesh::printsh(face* sface)
{
  ...
  if (sub2badface(*sface)) {
    printf(" (queued)");
  }
  if (sapex(*sface) == NULL) {
    if (segmenttype(*sface) == SHARPSEGMENT) {
      printf(" (sharp)");
    }
  }
  printf("\n");
...
///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// projpoint()    Return the projection point from a point to a line.        //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////

void tetgenmesh::projpoint(REAL* p, REAL* e1, REAL* e2, REAL* prj)
{
  REAL v1[3], v2[3];
  REAL len, l_p;
...
void tetgenmesh::initializepointpool()
{
  ...
  if (b->plc || b->refine) {
    // Increase the point size by an element pointer. It is aligned to a
    //   sizeof(tetrahedron)-byte address.
    pointsize = (point2simindex + 1) * sizeof(tetrahedron);
  } else {
...
void tetgenmesh::initializetetshpools()
{
  ...
  if (b->neighbors && (elesize < 8 * sizeof(tetrahedron))) {
    elesize = 8 * sizeof(tetrahedron);
  }
  ...
  if (b->useshelles) {
    ...
    subfaces = new memorypool(12 * sizeof(shellface) + sizeof(int),
                              SUBPERBLOCK, POINTER, 8);
    // Initialize the pool of subsegments. The subsegment's record is same
    //   with subface.
    subsegs = new memorypool(12 * sizeof(shellface) + sizeof(int),
                             SUBPERBLOCK, POINTER, 8);
...
///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// unflipsos()    Rescure a non-localy Delaunay and unflipable boundary edge.//
//                                                                           //
// 'flipface' (abc) is an unflipable face, ab is the non-Delaunay edge. Look //
// if there exist two boundary faces abd and abe which are on the same facet //
// and both are hull faces, hence ab is an hull edge.                        //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////

void tetgenmesh::unflipsos(triface* flipface, queue* flipqueue)
{
  triface abc, abde;
  triface abdtet, baetet;
  triface spinface;
  face abdsh, baesh;
  point pa, pb, pc, pd, pe;
  REAL attrib, volume;
  ...
  spinface = abc;
  do {
    if (!fnextself(spinface)) {
      tspivot(spinface, abdsh);
      assert(abdsh.sh != dummysh);
      findedge(&abdsh, pa, pb);
      abdtet = spinface;
      break;
    }
  } while (apex(spinface) != pc);
  // Find bae.
  baesh.sh = dummysh;
  esym(abc, spinface);
  do {
    if (!fnextself(spinface)) {
      tspivot(spinface, baesh);
      assert(baesh.sh != dummysh);
      findedge(&baesh, pb, pa);
      baetet = spinface;
      break;
    }
  } while (apex(spinface) != pc);
  ...
  if ((abdsh.sh == dummysh) && (baesh.sh == dummysh)) {
    // ab is not a hull edge.
    return;
  }

  // abd, bae are hull face and belong to the same facet since ab is not a
  //   subsegment.  However, they may have different marker, don't matter.
  assert((abdsh.sh != dummysh) && (baesh.sh != dummysh));
  pd = sapex(abdsh);
  pe = sapex(baesh);
  // a, b, d, e are exactly locally non-convex or coplanar.
  ori = orient3d(pa, pb, pd, pe);
  assert(ori <= 0.0);
  // abd, bae should also be non-Delaunay or cospherical.
  sign = insphere(pa, pb, pc, pe, pd);
  assert(sign >= 0.0);

  if (b->verbose > 2) {
    printf("    Rescure unflipable edge (%d, %d) at (- %d) and (- %d).\n",
           pointmark(pa), pointmark(pb), pointmark(pd), pointmark(pe));
  }

  // Now rescure the unflipable edge ab by patching a tetrahedron abde.
  // Deattach abd, bae from their connecting tets abdtet, baetet;
  tsdissolve(abdtet);
  tsdissolve(baetet);
  // Create a new tet abde;
  maketetrahedron(&abde);
  setorg(abde, pa);
  setdest(abde, pb);
  setapex(abde, pd);
  setoppo(abde, pe);
  // Set the element attributes of the new tetrahedron 'edca'.
  for (i = 0; i < in->numberoftetrahedronattributes; i++) {
    attrib = elemattribute(abdtet.tet, i);
    setelemattribute(abde.tet, i, attrib);
  }
  // Set the volume constraint of the new tetrahedron 'edca' if the -ra
  //   switches are not used together. In -ra case, the various volume
  //   constraints can be spreaded very far.
  if (b->varvolume && !b->refine) {
    volume = volumebound(abdtet.tet);
    setvolumebound(abde.tet, volume);
  }
  // Bond abde to abdtet;
  bond(abde, abdtet);
  // Bond abde to baetet;
  ...
  // Perform a 2-to-2 flip on abd->eda and bae->deb.
  flip22sub(&abdsh, NULL);
  //   bond abdtet to abd->eda.
  findedge(&abdsh, pe, pd);
  assert(sapex(abdsh) == pa);
  sesymself(abdsh);
  abde.loc = 3;
  tsbond(abde, abdsh);
  //   bond baetet to bae->deb.
  findedge(&baesh, pd, pe);
  assert(sapex(baesh) == pb);
  sesymself(baesh);
  abde.loc = 2;
  tsbond(abde, baesh);
  // Bond dummytet to abde;
  // abde.loc = 2;
  dummytet[0] = encode(abde); 
  
  if (flipqueue) {
    // Add two faces of abde into flipqueue.
    abde.loc = 0;
    enqueueflipface(abde, flipqueue);
    abde.loc = 1;
    enqueueflipface(abde, flipqueue);
  }
}
...
//                                                                           //
// If the quality mesh step is starting,  (indicated by pools 'encsubsegs',  //
// 'encsubfaces' and 'badtetrahedrons' are not NULLs.)  we will check the    //
...
splitsubface(point newpoint, face* splitface, queue* flipqueue)
{
  ...
  // Set the boundary mark of the new subfaces.
  setmark(bcv, mark(abv));
  setmark(cav, mark(abv));  
...
void tetgenmesh::splitsubedge(point newpoint, face* splitsh, queue* flipqueue)
{
  ...
  if (ab.sh != dummysh) {
    ...
    setmark(vb, mark(av));
    setsegmenttype(vb, segmenttype(av));
...
insertsite(point newpoint, triface* searchtet, queue* flipqueue)
{
  ...
  if (searchtet->tet == (tetrahedron *) NULL) {
    ...
    searchtet->tet = dummytet;
    intersect = locate(newpoint, searchtet);
  } else {
...
bool tetgenmesh::removebadtet(triface *badtet, queue* flipqueue)
{
...
void tetgenmesh::incrflipinit(queue* insertqueue)
{
  ...
  points->traversalinit();
  // Get the first point 'pa'.
  pa = pointtraverse();

  // Get the second point 'pb', which is not identical with 'pa'.  For
  //   finding a better base edge, pa and pb should not too close by
  //   comparing its length with the 'longest'.
  pb = pointtraverse();
  while (pb != (point) NULL) {
    vab[0] = pb[0] - pa[0];
    vab[1] = pb[1] - pa[1];
    vab[2] = pb[2] - pa[2];
    ablen = sqrt(dot(vab, vab));
    if (ablen / longest <= b->epsilon) {
      // 'pb' is (or nearly ) identical to 'pa', skip it.
      insertqueue->push(&pb);
    } else {
      break;
    }
  ...
  while (pc != (point) NULL) {
    ...
    vbc[1] = pc[1] - pb[1];
    vbc[2] = pc[2] - pb[2];
    bclen = sqrt(dot(vbc, vbc));
    if (bclen / longest > b->epsilon) {
      det = dot(vab, vbc) / (ablen * bclen);
      if (fabs(fabs(det) - 1.0) <= b->epsilon) {
        det = 1.0; // It is assumed to be collinear with ab.
      }
    } else {
      det = 1.0; // It's collinear with ab..
    }
    if (det == 1.0) {
      // They are (or nearly) collinear, put it back to queue and skip it.
      insertqueue->push(&pc);
    } else {
      break;
    }
  ...
  if (pc == (point) NULL) {
    printf("\nAll points are colinear, no triangulation be constructed.\n");
    exit(1);
  }
  ...
  while (pd != (point) NULL) {
    ...
    if (det == 0.0) {
      // They are coplanar, put it back to queue and skip it.
      insertqueue->push(&pd);
    } else {
      break;
    }
  ...
  // Add the rest of vertices into 'insertqueue'.
  pointloop = pointtraverse();
  while (pointloop != (point) NULL) {
...
long tetgenmesh::incrflipdelaunay()
{
  ...
  while (!insertqueue->empty()) {
    ...
    insres = insertsite(pointloop, &starttet, flipqueue);
...
locatesub(point searchpt, face* searchsh, point abovept)
{
  ...
  if (abovept == (point) NULL) {
    liftpoint = getliftpoint(mark(*searchsh));
...
long tetgenmesh::flipsub(queue* flipqueue)
{
  ...
  while ((qedge = (badface *) flipqueue->pop()) != NULL) {
    ...
    pc = sapex(flipedge);
    pd = sapex(symedge);
    liftpoint = getliftpoint(mark(flipedge)); 
...
void tetgenmesh::delaunayfixup(face* fixupsh, int leftside)
{
  ...
  liftpoint = getliftpoint(mark(*fixupsh));
...
void tetgenmesh::constrainededge(face* startsh, point tend)
{
  ...
  liftpoint = getliftpoint(mark(*startsh));
  tstart = sorg(*startsh);
...
///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// triangulatefacet()    Create a constrained Delaunay triangulation for a   //
//                       given facet.                                        //
//                                                                           //
// 'facetidx' is the index of the facet in 'in->facetlist' (starts from 1),  //
...
void tetgenmesh::mergefacets(queue* flipqueue)
{
  ...
  while (segloop.sh != (shellface *) NULL) {
    ...
    if (parentsh.sh != neighsh.sh && parentsh.sh == neineighsh.sh) {
      ...
      fidx1 = mark(parentsh) - 1;
      fidx2 = mark(neighsh) - 1;
    ...
    for (i = 0; i < arraysize; i++) {
      ...
      for (j = i + 1; j < arraysize; j++) {
        ...
        if (intersect == INTERSECT || intersect == SHAREFACE) {
          ...
          (*internum)++; 
          // For output them, infect these two faces (although they may
          //   already be infected).
          sinfect(sface1);
          sinfect(sface2);
        }
...
///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// markacutevertices()    Mark each segment vertex be acute or non-acute.    //
//                                                                           //
      ...
      if (checkpoint != tend) {
        ...
        if (checkpoint != (point) NULL) {
          ...
          if (iscollinear(p1, p2, refpoint, b->epsilon)) {
            ...
            success = insertsite(splitpoint, &searchtet, flipqueue);
...
///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// constrainedfacets()    Forming a constrained Delaunay tetrahedralization  //
//                        by inserting PLC facets into the DT.               //
//                                                                           //
...
void tetgenmesh::indenthull()
{
  ...
  viri = new memorypool(sizeof(tetrahedron *), VIRUSPERBLOCK, POINTER, 0);
...
long tetgenmesh::reconstructmesh()
{
    ...
    for (i = 0; i < in->numberoftrifaces; i++) {
      ...
      if (bondflag) {
        ...
        if (in->trifacemarkerlist != (int *) NULL) {
          setmark(subloop, in->trifacemarkerlist[i]);
        }
        adjustedgering(neightet, CCW);
      ...
      if (neighsh.sh == dummysh) {
        ...
        for (j = idx2facelist[iorg]; j < idx2facelist[iorg + 1]; j++) {
          ...
          if (isfacehasedge(&neighsh, torg, tdest)) {
            ...
            /*
            for (index = 0; index < neighshlist->len(); index++) {
              neineighsh = * (face *)(* neighshlist)[index];
              napex = sapex(neineighsh);
              sign = orient3d(torg, tdest, tapex, napex);
              if (sign > 0.0) {
                break; // Insert it before 'index'.
              }
            }
            neighshlist->insert(index, &neighsh);
            */
          }
        }
      ...
      if (subseg.sh == dummysh) {
        ...
        if (subloop.sh == neighsh.sh || subloop.sh != neineighsh.sh) {
          ...
          if (iscoplanar(torg, tdest, tapex, napex, sign, b->epsilon)) {
            ...
            insertsegflag = (mark(subloop) != mark(neighsh));
          } else {
            // Non-coplanar.
            insertsegflag = true;
          }
        ...
        /*
        if (insertsegflag && (subloop.sh != neineighsh.sh)) {
          // If there are more than two subfaces, the direction of the
          //   subsegment must conform to the face ring (right-hand rule).
          napex = sapex(neighsh);
          sign = orient3d(torg, tdest, tapex, napex);
          if (sign > 0.0) {
            // The direction of 'subloop' is reverse to face ring.
            norg = torg; torg = tdest; tdest = norg;
          }
        }
        */ 
        if (insertsegflag) {
          ...
          // Temporarily set the vertices type be INPUTVERTEX.
          setpointtype(torg, INPUTVERTEX);
          setpointtype(tdest, INPUTVERTEX);
  ...
  // Set the correct types (ACUTE or NONACUTE) of all segment vertices.
  markacutevertices(89.0);
        ...
        for (j = 0; j < 3; j++) {
          ...
          if (subseg.sh == dummysh) {
            ...
            if (!sinfected(neineighsh)) {
              ...
              assert(mark(neineighsh) == marker);
              setmark(neineighsh, facetidx);
              sinfect(neineighsh);
  ...
  /*
  // Discard points which are not corners of tetrahedra.
  for (i = 0; i < in->numberofpoints; i++) {
    if (idx2tetlist[i] == idx2tetlist[i + 1]) {
      // A dead point.
      pointloop = idx2verlist[i];
      pointdealloc(pointloop);
    }
  }
  // Renumber point index if there are discarded points.
  if (points->items < in->numberofpoints) {
    numbernodes();
  }
  */
...
void tetgenmesh::insertaddpoints()
{
  ...
  for (i = 0; i < in->numberofaddpoints; i++) {
    ...
    if (loc != ONVERTEX) {
      epspp = b->epsilon;
      while (1) {
        loc = adjustlocate(newpoint, &searchtet, OUTSIDE, epspp);
        if (loc == ONVERTEX) {
          checkpt = org(searchtet);
          if ((newpoint[0] != checkpt[0]) || (newpoint[1] != checkpt[1]) ||
                (newpoint[2] != checkpt[2])) {
            epspp *= 1e-2;
            continue;
          }
        }
    ...
    if (loc == OUTSIDE) {
      ...
      while (searchtet.tet != (tetrahedron *) NULL) {
        ...
        if (iscoplanar(p2, p1, p3, newpoint, ori, b->epsilon)) ori = 0.0;
        if (ori >= 0) {
          ori = orient3d(p1, p2, p4, newpoint);
          if (iscoplanar(p1, p2, p4, newpoint, ori, b->epsilon)) ori = 0.0;
          if (ori >= 0) {
            ori = orient3d(p2, p3, p4, newpoint);
            if (iscoplanar(p2, p3, p4, newpoint, ori, b->epsilon)) ori = 0.0;
            if (ori >= 0) {
              ori = orient3d(p3, p1, p4, newpoint);
              if (iscoplanar(p3, p1, p4, newpoint, ori, b->epsilon)) ori = 0.0;
              if (ori >= 0) {
                ...
                while (1) {
                  ...
                  if (loc == ONVERTEX) {
                    checkpt = org(searchtet);
                    if ((newpoint[0] != checkpt[0]) ||
                        (newpoint[1] != checkpt[1]) ||
                        (newpoint[2] != checkpt[2])) {
                      epspp *= 1e-2;
                      continue;
                    }
...
///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// checktet4illtet()    Check a tetrahedron to see if it is ill.             //
//                                                                           //
// A tetrahedron is ill if: (1) it has zero volume, i.e., it's four vertices //
// are exactly coplanar; (2) it spans one facet, i.e., it has two adjacent   //
// faces lie in the same facet. Case (1) may happen since the CDT algorithm  //
// may create it. While case (2) is caused by the vertices of the facet are  //
// not exactly coplanar.                                                     //
//                                                                           //
// Return TRUE if the tetrahedron is ill.  When it is, add it into list if   //
// 'enqflag' is TRUE.                                                        //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////

bool tetgenmesh::checktet4illtet(triface* testtet)
{
  ...
  if (isill) {
    badtet = (badtetrahedron *) illtetrahedrons->alloc();
    badtet->tet = *testtet;
...
void tetgenmesh::tallilltets()
{
  triface tetloop;
  REAL bakdihedral;

  bakdihedral = b->maxdihedral;
  b->maxdihedral = PI * (1.0 - b->epsilon);

  tetrahedrons->traversalinit();
  tetloop.tet = tetrahedrontraverse();
  while (tetloop.tet != (tetrahedron *) NULL) {
    if (!checktet4illtet(&tetloop)) {
      checktet4sliver(&tetloop);
    }
    tetloop.tet = tetrahedrontraverse();
  }
...
void tetgenmesh::removeilltets()
{
  ...
  illtetrahedrons = new memorypool(sizeof(badtetrahedron), BADELEPERBLOCK,
                                   POINTER, 0);
  // Initialize the pool of recently flipped faces.
  flipstackers = new memorypool(sizeof(flipstacker), FLIPSTACKERPERBLOCK,
                                POINTER, 0);
  // Initialize 'flipqueue'.
  flipqueue = new queue(sizeof(badface));  
  ...
  tallilltets();
  illtetrahedrons->traversalinit();
  badtet = (badtetrahedron *) illtetrahedrons->traverse();
  while (badtet != (badtetrahedron *) NULL) {
    if (!isdead(&badtet->tet) && (org(badtet->tet) == badtet->tetorg) &&
      ...
      removebadtet(&badtet->tet, flipqueue);
    }
    badtet = (badtetrahedron *) illtetrahedrons->traverse();
  }
  ...
  delete flipqueue;
  delete flipstackers;
  delete illtetrahedrons;
}

//
// End of mesh repair rouitnes
//

//
// Begin of conforming Delaunay routines
//
...
// Local Feature Size                                                        //
//                                                                           //
// For a PLC X, the mesh domain M of X is bounded by facets of X.  Vertices, //
// segments and subfaces of X are called boundaries of M.  The local feature //
// size is a function, lfs() defined for all points of M. For any point x of //
// M,  lfs(x) is the radius of the smallest ball centered at x intersecting  //
// two non-adjacent boundaries.  The lfs() is a continuous function that is  //
// positive everywhere in M. It suggests that how big the empty space around //
// a point in M. Note the local feature size only depends on the input X and //
// does not change when new vertices are inserted.                           //
//                                                                           //
// Acute Vertices and Protecting Spheres                                     //
//                                                                           //
// A vertex is acute if at least two segments connecting at it form an angle //
// smaller than 90 degree.  To split a segment (it's encroached, hence non-  //
// conforming) having acute vertex need to take special care. Since the new  //
// inserted vertex may encroach upon an other segment sharing at the same    //
// acute vertex, and to split the new encroached segment may cause the prev- //
// iously split segment be encroached again, and the process may never end.  //
//                                                                           //
// To avoid this effect, suppose every acute vertex is protected by a sphere,//
// new inserted vertex should not lie inside this sphere, hence no segment   //
// inside this sphere will be encroached.  The key question is to choose a   //
// suitable radius of the protecting sphere. We choose the radius as follows://
// Let p be an acute vertex, let s be a segment need to be split, s contains //
// p.  The radius of the protecting sphere S of p is R, R = lfs(p) / 2^n and //
// R <= |s| / 2.  The split point on s is the intersecting point of S and s. //
//                                                                           //
// We use an array "rpsarray" to keep the current protecting radii for all   //
// acute vertices (indexed by pointmark()).  Initially, the array contains   //
// the local feature sizes of all acute vertices.                            //
//                                                                           //
// Sharp Segments and Protecting Cylinders                                   //
//                                                                           //
// A segment is said to be sharp if at least two facets intersecting at it   //
// form a dihedral angle smaller than 90 degree.  To split a subface (it's   //
// encroached and non-conforming) of a facet containing a sharp segment need //
// to take special care.  Each sharp segment is protected by a cylinder      //
// around it.  The initial radius of the cylinder is choosed to be the       //
// smallest local feature size among points of this segment.                 //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// checkseg4encroach()    Check if a segment is encroached.                  //
//                                                                           //
// A segment is encroached if there is a vertex lies inside its diametric    //
// circumsphere. That is, the segment faces an angle greater than 90 degree. //
//                                                                           //
// 'testseg' is the segment needs to be check. If 'testpt' is NULL, test all //
// apexes of the faces containing 'testseg', otherwise, only test 'testpt'.  //
// If any of them encroaches on it, return TRUE, and if 'enqflag' is TRUE.   //
// It will be queued in 'encsubsegs'.                                        //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////

bool tetgenmesh::checkseg4encroach(face* testseg, point testpt, bool enqflag)
{
  badface *encsubseg;
  ...
  if (testpt == (point) NULL) {
    // Test all apexes of the faces containing 'testseg'.  It may happen
    //   that some tetrahedra containing 'testseg' (ab) are completely
    //   disconnected with others. If it happens, use the face link of ab
    //   to cross the boundary.
    sstpivot(testseg, &starttet);
    spintet = starttet;
    eapex = apex(spintet);
    do {
      dist = distance(cent, eapex);
      diff = dist - radius;
      // Do rounding if dist is very very close to radius.
      if (fabs(diff) / radius <= b->epsilon) {
        diff = 0.0;  // Round to zero.
      }
      if (diff < 0.0) {
        enq = true;
        break;
      }
      // Get the next face containing 'testseg'.
      if (!fnextself(spintet)) {
        // Meet a boundary, walk through it.
        tspivot(spintet, spinsh);
        assert(spinsh.sh != dummysh);
      ...
      eapex = apex(spintet);
    } while (apex(starttet) != eapex); 
  } else {
    // Test if 'testpt' is encroaching 'testseg'.
  ...
  if (enq && enqflag) {
    if (b->verbose > 2) {
      printf("    Queuing encseg x%lx (%d, %d).\n", 
             (unsigned long) (testseg->sh), pointmark(eorg), pointmark(edest));
    }
    encsubseg = (badface *) encsubsegs->alloc();
    ...
    setsub2badface(encsubseg->ss, encsubseg);
  }
...
void tetgenmesh::tallencsegs(point testpt, list *cavtetlist)
{
  triface starttet, neightet;
  face checkseg;
  int i, j;
...
///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// tallencsubs()    Traverse the entire list of subfaces, and check each to  //
//                  see if it is encroached.  If so, add it to the list.     //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////

void tetgenmesh::tallencsubs()
{
  face subfaceloop;
  
  subfaces->traversalinit();
  subfaceloop.sh = shellfacetraverse(subfaces);
  while (subfaceloop.sh != (shellface *) NULL) {
    checksub4encroach(&subfaceloop, NULL, true);
    subfaceloop.sh = shellfacetraverse(subfaces);
  }
}
...
///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// splitencsegs()    Split all the encroached segments.                      //
//                                                                           //
...
// existing segments; these are also repaired.                               //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////

void tetgenmesh::splitencsegs(queue* flipqueue)
{
  badface *encloop;
  triface starttet;
  face splitseg;
  ...
  // Loop until the list 'encsubsegs' is empty.
  while (encsubsegs->items > 0) {
    encsubsegs->traversalinit();
    encloop = badfacetraverse(encsubsegs);
    while (encloop != (badface *) NULL) {
      splitseg = encloop->ss;
      // Every splitseg has a pointer to encloop, now clear it.
      assert(sub2badface(splitseg) == encloop);
      setsub2badface(splitseg, NULL);
      eorg = sorg(splitseg);
      edest = sdest(splitseg);
      assert((eorg == encloop->forg) && (edest == encloop->fdest));

      if (b->verbose > 1) {
        printf("  Getting encseg (%d, %d).\n", pointmark(eorg),
               pointmark(edest));
      }
      ...
      if (acuteorg || acutedest) {
        ...
        rps = rpsarray[ptidx];
        // Calculate the suitable radius to split the segment. It should be
        //   no larger than half of the segment length.
        while (rps > 0.51 * len) {
          rps *= 0.5;
        }
        if (rps < rpsarray[ptidx]) {
          // Update (shrink) the radius of current protecting sphere.
          rpsarray[ptidx] = rps;
        }
      ...
      // Set the unique index of the new point
      ptidx = (int) points->items - (in->firstnumber == 1 ? 0 : 1);
      setpointmark(newpoint, ptidx);
      // Set the type of the new point.
      setpointtype(newpoint, FREESEGVERTEX);
      ...
      badfacedealloc(encsubsegs, encloop);  
...
// Notations                                                                 //
//                                                                           //
// Let abc denote a face with origin a, destination b and apex c;  let ab    //
// denote an edge with two endpoints a and b. |ab| is the edge length.       //
//                                                                           //
// Let H(abc) denote the plane containing abc;  F(abc) denote the facet that //
// abc belongs to.                                                           //
//                                                                           //
// Let DS(abc) denote the diametric circumsphere of abc. If a is acute, let  //
// PS(a) denote the protecting sphere of a.                                  //
//                                                                           //
// If ab is a sharp segment, let CL(ab) denote the protecting cylinder of ab.//
//                                                                           //
///////////////////////////////////////////////////////////////////////////////

///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// marksharpsegments()    Mark each segment be sharp or non-sharp.           //
//                                                                           //
// Parameter 'acuteangle' gives the upperbound (in degree). Angles which are //
// smaller or equal than it are assumed as acute angles.  A segment is sharp //
// if at least two facets incident at it with a dihedral angle be acute.     //
//                                                                           //
// An input segment may have been split into several subsegments.  They are  //
// connected through pointers.  We only mark the input segment be sharp or   //
// non-sharp, all the subsegments are the same. At the same time, each input //
// segment is set a unique index (starting from 0).  All subsegments of the  //
// input segment have the same index.                                        //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////

void tetgenmesh::marksharpsegments(REAL acuteangle)
{
  list *incishlist;
  triface adjtet;
  face segloop, prevseg, nextseg;
  face parentsh, spinsh, neighsh;
  point eorg, edest;
  ...
  // The small dihedral angle bound (90 degrees) in arcs.
  anglebound = acuteangle * PI / 180.0;
  // Initialize a working list.
  incishlist = new list(sizeof(face), NULL);
  // Initialize the segment index (starting from 0).
  segidx = 0;

  // Loop through the list of segments, mark each segment (be sharp or non-
  //   sharp), and set an index to it.  
  subsegs->traversalinit();
  segloop.sh = shellfacetraverse(subsegs);
  while (segloop.sh != (shellface *) NULL) {
    // Only operate on the segment if it contains the origin.
    segloop.shver = 0;
    ...
    if (prevseg.sh == dummysh) {
      ...
      if (spinsh.sh != parentsh.sh) {
        ...
        for (i = 1; i <= incishlist->len(); i++) {
          ...
          // Only do test when the side of spinsh is faceing inward.
      ...
      // Set the index and sharp flag to all the subsegments.
      nextseg = segloop;
      do {
        nextseg.shver = 0;
        setmark(nextseg, segidx);
        if (issharp) {
          setsegmenttype(nextseg, SHARPSEGMENT);
        } else {
          setsegmenttype(nextseg, NONSHARPSEGMENT);
        }
        senextself(nextseg);
        spivotself(nextseg);
      } while (nextseg.sh != dummysh);
      // Increment the index for the next input segment. 
      segidx++;
    }
    segloop.sh = shellfacetraverse(subsegs);
  }
  assert(segidx <= (int) insegment);
...
///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// initializeapcarray()    Calculate the protecting angles of sharp segments,//
//                         save in 'apcarray'.                               //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////

void tetgenmesh::initializeapcarray()
{
  face parentsh, spinsh;
  face segloop, lseg;
  point forg, fdest, fapex;
  REAL smallangle, angle;
  REAL fnormal[3], len;
  bool issharp;
  int segidx;
  int i, j;
  ...
  if (b->verbose) {
    printf("  Initializing protecting angles.\n");
  }

  // First initialize all entries to be zero.
  for (i = 0; i < insegment; i++) {
    apcarray[i * 2] = apcarray[i * 2 + 1] = 0.0;
  }
  ...
  while (segloop.sh != (shellface *) NULL) {
    segidx = mark(segloop);
    // Is it a sharp segments.
    if (segmenttype(segloop) == SHARPSEGMENT) {
      // Test the two endpoints of this segment.
      segloop.shver = 0;
      for (i = 0; i < 2; i++) {
        if (i == 1) {
          sesymself(segloop);
        ...
        forg = sorg(segloop);
        // Is it an acute vertex?
        if (pointtype(forg) == ACUTEVERTEX) {
          fdest = sdest(segloop);
          spivot(segloop, parentsh);
          if (sorg(parentsh) != forg) {
            sesymself(parentsh);
          }
          assert((sorg(parentsh) == forg) && (sdest(parentsh) == fdest));
          smallangle = PI;
          spinsh = parentsh;
          do {
            // The right edge (forg, fdest) of spinsh is a subsegemnt. Get
            //    the segment at the left side of spinsh containing forg.
            //    It may not be the left edge (forg, apex(spinsh)).
            getorlsegments(&spinsh, NULL, &lseg);
            if (sorg(lseg) != forg) sesymself(lseg);
            assert(sorg(lseg) == forg);
            fapex = sdest(lseg);
            // Get the normal of spinsh.
            facenormal(forg, fdest, sapex(spinsh), fnormal, &len);
            for (j = 0; j < 3; j++) fnormal[j] /= len;
            // Get the angle between (forg, fdest) and (forg, fapex).
            angle = interiorangle(forg, fdest, fapex, fnormal);
            // Check if lseg is sharp;
            issharp = (segmenttype(lseg) == SHARPSEGMENT);
            if (issharp) {
              angle /= 3.0;
            } else {
              angle /= 2.0;
            ...
            // Get the next subface containing this segment.
            sfnextself(spinsh);
          } while (spinsh.sh != parentsh.sh);
          // Save the protecting angle.
          apcarray[segidx * 2 + i] = smallangle;
        } 
      }
    }
    segloop.sh = shellfacetraverse(subsegs);
...
///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// getorlsegments()    Get the two subsegments at the right and left sides   //
//                     and containing the origin of a given subface.         //
//                                                                           //
// 'encsub' is a subface, and we know its origin o is an inserting vertex to //
// split a (sub)segment e1e2 (i.e., it has type FREESEGVERTEX).              //
//                                                                           //
// This routine find the two subsegments e1o and e2o.  They may not be the   //
// two edges of 'encsub'.  'lseg' and 'rseg' are used to return the segments.//
// Assume, the encsub is oriented in CCW direction, 'lseg' is the one at the //
// left side of it, 'rseg' is at the right side.  If 'rseg' or 'lseg' is set //
// to NULL, it is ignored.                                                   //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////

void tetgenmesh::getorlsegments(face* encsub, face* rseg, face* lseg)
{
  face checksh, checkseg;
  point forg, fdest;
  int i;
  ...
  for (i = 0; i < 2; i++) {
    checksh = *encsub;
    if (i == 0) {
      if (rseg == (face *) NULL) continue; // Skip rseg.
    } else {
      if (lseg == (face *) NULL) continue; // Skip lseg.
      // Find the segment at the left side.
      senext2self(checksh);
      sesymself(checksh);
      assert(sorg(checksh) == forg);
    }
    // Find the segment containing forg.
    while (true) {
      // Check if there is a segment attaching this edge.
      sspivot(checksh, checkseg);
      if (checkseg.sh != dummysh) break;
      // No segment! Go to another edge containing forg, it belongs to a
      //   different subface.
      fdest = sdest(checksh);
      spivotself(checksh);
      // It should always meet a segment before come back.
      assert(checksh.sh != encsub->sh);
      // Adjust the edge dir so that it is directing from fdest to forg.
      if (sorg(checksh) != fdest) {
        sesymself(checksh);
        assert(sorg(checksh) == fdest);
      }
      // Now go to the edge we want to check.
      senextself(checksh);
      assert(sorg(checksh) == forg);
    }
    if (i == 0) {
...
///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// getrpc()    Get the radius of the CL(s) at the origin of a given subface. //
//                                                                           //
// 'encsub' is an encroached subface and its origin lies on a sharp segment. //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////

REAL tetgenmesh::getrpc(face* encsub)
{
  face startsh, spinsh, checksh;
  face checkseg;
  point forg, fdest, fapex, workpt;
  REAL jmpo[3], mid[3], jmpm[3];
  REAL orthonorm[3], p[7], ip[3];
  REAL rpc, dist, len, angle;
  bool isprotpt;
  int i;  

  // Set startsh, where the origin of startsh is the origin of encsub, the
  //   edge of startsh is a sharp segment.
  forg = sorg(*encsub);
  checksh = *encsub;
  while (true) {
    // Check if there is a segment attaching this edge.
    ...
    assert(checksh.sh != encsub->sh);
    // Adjust the edge dir so that it is directing from fdest to forg.
    if (sorg(checksh) != fdest) sesymself(checksh);
    assert(sorg(checksh) == fdest);
    // Now go to the edge we want to check.
    senextself(checksh);
    assert(sorg(checksh) == forg);
  ...
  startsh = checksh;
  assert(segmenttype(checkseg) == SHARPSEGMENT);
  fdest = sdest(checksh);
  len = distance(forg, fdest);  

  // Calculate the radius (rpc) of CL(s).
  rpc = longest;
  spinsh = startsh;
  do {
    fapex = sapex(spinsh);
    edgeorthonormal(forg, fdest, fapex, orthonorm);
    for (i = 0; i < 3; i++) jmpo[i] = forg[i] + len * orthonorm[i];
    // Loop through edges which have one end be forg.
    checksh = spinsh;
    do {
      senext2self(checksh);
      sspivot(checksh, checkseg);
      // If this edge is a segment, stop;
      if (checkseg.sh != dummysh) break;
      spivotself(checksh);
      if (sorg(checksh) != forg) sesymself(checksh);
      assert(sorg(checksh) == forg);
      // Get the dest of checksh.
      workpt = sdest(checksh);
      // Is workpt a parent point of forg?
      isprotpt = (point2ppt(workpt) == forg);
      if (isprotpt) {
        // Use this point.
        assert(pointtype(workpt) == FREESUBVERTEX);
        rpc = distance(forg, workpt);
        if (spinsh.sh == startsh.sh) {
          // Find a protetcing point on the splitting subface.
          rpc *= 0.5;
        }
        assert(rpc < longest);
        return rpc;
      }
      // Get the angle between (forg, jmpo) and (forg, workpt).
      angle = interiorangle(forg, jmpo, workpt, NULL);
      // Only do linelineint() when the two edges (forg, jmpo) and (forg,
      //   workpt) are not orthogonal. linelineint() has roundoff error.
      if (fabs(0.5 * PI - angle) > (b->epsilon * 1e+2)) {
        // These two lines are not (nearly) parallel. 
        for (i = 0; i < 3; i++) mid[i] = 0.5 * (forg[i] + workpt[i]);
        edgeorthonormal(forg, workpt, sapex(checksh), orthonorm);
        for (i = 0; i < 3; i++) jmpm[i] = mid[i] + len * orthonorm[i];
        // Get the intersecting line segment of two lines.
        linelineint(forg, jmpo, mid, jmpm, p);
        assert(p[0] > 0.0);
        // Get the midpoint of the intersecting line segment.
        for (i = 0; i < 3; i++) ip[i] = 0.5 * (p[1 + i] + p[4 + i]);
        dist = distance(forg, ip);
        // If dist is smaller than rpc, update rpc.
        if (rpc > dist) rpc = dist; 
      }
    } while (true);
    // Get the next subface containing the sharp segment (forg, fdest).
    sfnextself(spinsh);
...
///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// categorizeencsub()    Categorize the type of an encroached subface.       //
//                                                                           //
// 'encsub' is an encroached subface. This routine finds what type it is, so //
// it can be split by a corresponding method. The return value indicates the //
// type, 'encsub' will be set properly.                                      //
//                                                                           //
// Following are the possible returning values:                              //
//   - Return ACUTEVSHARPS if the subface has an acute vertex and a sharp    //
//     segment connecting to it.  The orign of 'encsub' is acute and the     //
//     primary edge of 'encsub' is sharp.                                    //
//   - Return ACUTEV if the subface has an acute vertex (ACUTEVERTEX).       //
//     The origin of 'encsub' represents the acute vertex.                   //
//   - Return SHARPS if the subface is not in type ACUTEV. It has an edge    //
//     which is or be a part of a sharp segment.  This edge is represented   //
//     by the primary edge of 'encsub', 'sharpseg' returns the segment.      //
//   - Return FSVSHARPS if the subface is in none of the above types. It has //
//     a corner which has the type FREESEGVERTEX and lies on a sharp segment.//
//     The origin of 'encsub' represents the vertex,  'sharpseg' returns the //
//     segment.                                                              //
//   - Return NAVSHARPS if the subface is in none of the above types. It has //
//     a corner which has the type NONACUTEVERTEX and is the end of a sharp  //
//     segment. The origin of 'encsub' represents the vertex,  'sharpseg'    //
//     returns the segment.                                                  //
//   - Return NAVNSHARPS if the subface is in none of the above types.       //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////

enum tetgenmesh::encsubtype tetgenmesh::categorizeencsub(face* encsub)
{
  face checksh;
  face rseg, lseg;
  point forg;
  ...
  // Is there an acute vertex?
  for (i = 0; i < 3; i++) {
    forg = sorg(*encsub);
    if (pointtype(forg) == ACUTEVERTEX) {
      // Is there a sharp segment?
      sspivot(*encsub, rseg);
      if (rseg.sh != dummysh) {
        if (segmenttype(rseg) == SHARPSEGMENT) {
          return ACUTEVSHARPS;
        }
      }
      senext2(*encsub, checksh);
      sspivot(checksh, rseg);
      if (rseg.sh != dummysh) {
        if (segmenttype(rseg) == SHARPSEGMENT) {
          // Adjust encsub.
          *encsub = checksh;
  ...
  // Is there a sharp segment containing one of encsub's vertices?
  for (i = 0; i < 3; i++) {
    forg = sorg(*encsub);
    if (pointtype(forg) == FREESEGVERTEX) {
      // Find the segment containing forg.
      getorlsegments(encsub, &rseg, NULL);
      // Is checkseg sharp?
      if (segmenttype(rseg) == SHARPSEGMENT) {
        return FSVSHARPS;
      }
  ...
  // Is there a sharp seg has an endpoint be one of encsub's vertices?
  bool rsharp, lsharp;
  for (i = 0; i < 3; i++) {
    forg = sorg(*encsub);
    if (pointtype(forg) == NONACUTEVERTEX) {
      // Search for a sharp segment containing forg.  Here the difference
      //   from above is there are two segments containing forg. We need
      //   to check both if it is necessary.
      getorlsegments(encsub, &rseg, &lseg);
      // Is there checkseg sharp?
      rsharp = (segmenttype(rseg) == SHARPSEGMENT);
      lsharp = (segmenttype(lseg) == SHARPSEGMENT);
      if (rsharp || lsharp) {
        return NAVSHARPS;
      }
    }
  ...
  // No acute vertex, no sharp segment.
  return NAVNSHARPS;
}

///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// checksub4encroach()    Check if a subface is encroached.                  //
//                                                                           //
// A subface is encroached if there is a vertex lies inside its diametric    //
// circumsphere.  'testsub' is the subface needs to be check.  If it is      //
// encroached, return TRUE, and if 'enqflag' is TRUE, it will be queued.     //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////
...
bool tetgenmesh::checksub4encroach(face* testsub, point testpt, bool enqflag)
{
  ...
  if (!ncollinear) {
    // forg, fdest, and fapex are collinear.  This may happen when a
    //   newpoint was inserted very close to an edge of a subface. Such
    //   nearly degenerate subface should only exist temporarily, the
    //   consequent flips will finally remove them.
    return false;
  }
  ...
  if (enq && enqflag) {
    // Determine the appropriate queue to put the encsub.
    ety = categorizeencsub(testsub);
    // Allocate space from the pool.
    encsub = (badface *) encsubfaces->alloc();
    // Set the data.
    encsub->ss = *testsub;
    ...
    // Put encsub into the appropriate queue.
    quenumber = (int) ety;    
    if (b->verbose > 2) {
      printf("    Queuing encsub x%lx (%d, %d, %d) [%d].\n",
             (unsigned long) encsub, pointmark(encsub->forg),
             pointmark(encsub->fdest), pointmark(encsub->fapex), quenumber);
    }
    // Let the subface remember the encsub.
    setsub2badface(encsub->ss, encsub);
    // Queue elements are double-linked.
    encsub->prevface = (badface *) NULL;
    encsub->nextface = (badface *) NULL;
    // Add the element to the end of a queue.
    if (subquetail[quenumber] == (badface *) NULL) {
      // This queue is empty.  It is the only element in the link.
      subquefront[quenumber] = subquetail[quenumber] = encsub;
    } else {
      // Add it at the end of the queue.
      assert(subquetail[quenumber]->nextface == (badface *) NULL);
      subquetail[quenumber]->nextface = encsub;
      encsub->prevface = subquetail[quenumber];
...
///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// doencchecktest()    Check if we need to do encroach test on 'testsub'.    //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////

void tetgenmesh::doencchecktest(face* testsub)
{
  badface *bface, *prevnode, *sucnode;
  int quenumber;

  // Is testsub in the queue?
  bface = sub2badface(*testsub);
  if (bface != (badface *) NULL) {
    // Is it changed by flips?
    if ((sorg(bface->ss) != bface->forg) || (sdest(bface->ss) != bface->fdest)
        || (sapex(bface->ss) != bface->fapex)) {
      if (b->verbose > 2) {
        printf("    Removing encsub x%lx (%d, %d, %d).\n",
               (unsigned long) bface, pointmark(bface->forg),
               pointmark(bface->fdest), pointmark(bface->fapex));
      }
      if (bface->prevface == (badface *) NULL) {
        // This is the first element of a queue. Find the quenumber.
        for (quenumber = 0; quenumber < 6; quenumber++) {
          if (subquefront[quenumber] == bface) break;
        }
        assert(quenumber < 6);
        sucnode = bface->nextface;
        if (sucnode == (badface *) NULL) {
          // It is the only node in the queue. Empty the queue.
          subquefront[quenumber] = subquetail[quenumber] = (badface *) NULL;
        } else {
          subquefront[quenumber] = sucnode;
          sucnode->prevface = (badface *) NULL;
        }
      } else {
        // It is not the first element. (Note, it may be the last one!)
        prevnode = bface->prevface;
        sucnode = bface->nextface;
        prevnode->nextface = sucnode;
        if (sucnode == (badface *) NULL) {
          // This is the last node, update the pointer in subquetail[].
          for (quenumber = 0; quenumber < 6; quenumber++) {
            if (subquetail[quenumber] == bface) break;
          }
          assert(quenumber < 6);
          subquetail[quenumber] = prevnode;
        } else {
          // It is not the last node.
          sucnode->prevface = prevnode;
        } 
      }
      // Clear the pointer in testsub.
      setsub2badface(*testsub, NULL);
      // Remove this badface from list.
      badfacedealloc(encsubfaces, bface);
    } else {
      // It is encroached and queued. Don't need to check.
      return;
    }
  }

  // Otherwise, we need to check it.
  checksub4encroach(testsub, NULL, true);
}

///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// calctynavnss()    Get the splitting point of an encroached subface.       //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////

void tetgenmesh::calctynavnss(badface* bface, point newpoint)
{
  if (b->verbose > 2) {
    printf("    NAVNSHARPS: (%d, %d, %d).\n", pointmark(bface->forg),
           pointmark(bface->fdest), pointmark(bface->fapex));
  }
  // Choose the circumcenter as the splitting point.
  newpoint[0] = bface->cent[0];
  newpoint[1] = bface->cent[1];
  newpoint[2] = bface->cent[2];
  // There is no parent point available.
  setpoint2ppt(newpoint, NULL);
}

///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// calctyacutevsharps()    Get the splitting point of an encroached subface. //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////

void tetgenmesh::calctyacutevsharps(badface* bface, point newpoint)
{
  face sharpseg;
  point pa, pb, pc;
  REAL fnormal[3], tmp[3];
  REAL pe[3], rotangle;
  REAL rps, len, split;
  int ptidx, segidx, i;

  sspivot(bface->ss, sharpseg);
  assert(sharpseg.sh != dummysh);  
  segidx = mark(sharpseg);
  assert(segmenttype(sharpseg) == SHARPSEGMENT);
  pa = bface->forg;
  pb = bface->fdest;
  pc = bface->fapex;
  // pa is acute, pa, pb is a sharp segment.
  if (sorg(sharpseg) != pa) sesymself(sharpseg);
  assert((sorg(sharpseg) == pa) && (sdest(sharpseg) == pb));
  // Get the rotate angle.
  i = sharpseg.shver;
  assert((i == 0) || (i == 1));
  rotangle = apcarray[segidx * 2 + i];
  assert(rotangle > 0.0);
  
  if (b->verbose > 2) {
    printf("    ACUTEVSHARPS: (%d, %d, %d) rotan = %g.\n", pointmark(pa),
           pointmark(pb), pointmark(pc), rotangle / PI * 180.0);
  }

  // Get the normal of abc, n = ab x ac.
  facenormal(pa, pb, pc, fnormal, &len);
  for (i = 0; i < 3; i++) fnormal[i] /= len;
  ptidx = pointmark(pa) - in->firstnumber;
  rps = rpsarray[ptidx];
  len = distance(pa, pb);
  split = rps / len;
  if (fabs(1.0 - split) <= b->epsilon) {
    for (i = 0; i < 3; i++) pe[i] = pb[i];
  } else {
    for (i = 0; i < 3; i++) pe[i] = pa[i] + split * (pb[i] - pa[i]);
  }
  for (i = 0; i < 3; i++) tmp[i] = pa[i] + rps * fnormal[i];
  // Rotate e around a away from ab with an angle rotangle. The rotating
  //   axis is passing a and parallel to the normal of abc.
  rotatepoint(pe, rotangle, pa, tmp);
  
  // Set the newpoint be pe.
  for (i = 0; i < 3; i++) newpoint[i] = pe[i];
  // a is the parent point of newpoint.
  setpoint2ppt(newpoint, pa);
}

///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// calctyacutev()    Get the splitting point of an encroached subface.       //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////

void tetgenmesh::calctyacutev(badface* bface, point newpoint)
{
  point pa, pb, pc;
  REAL pd[3], pe[3];
  REAL rps, len, split;
  ...
  // abc has type ACUTEV, and a is acute, let d be the center of DS(abc).
  pa = bface->forg;
  pb = bface->fdest;
  pc = bface->fapex;
  for (i = 0; i < 3; i++) pd[i] = bface->cent[i];
  // First calculate a point e at the intersection of PS(a) and ad.  
  idx = pointmark(pa) - in->firstnumber;
  rps = rpsarray[idx];
  len = distance(pa, pd);
  split =  rps / len;
  for (i = 0; i < 3; i++) pe[i] = pa[i] + split * (pd[i] - pa[i]);

  if (b->verbose > 2) {
    printf("    ACUTEV: (%d, %d, %d) rps = %g.\n", pointmark(pa),
           pointmark(pb), pointmark(pc), rps);
  }

  // Are b and c acute?
  if (pointtype(pb) == ACUTEVERTEX) {
    // b is acute, e should not lie inside PS(b).
    REAL rpsb, disteb;
    idx = pointmark(pb) - in->firstnumber;
    rpsb = rpsarray[idx];
    disteb = distance(pb, pe);
    assert(disteb >= rpsb);
  }
  if (pointtype(pc) == ACUTEVERTEX) {
    // c is acute, e should not lie inside PS(c).
    REAL rpsc, distec;
    idx = pointmark(pc) - in->firstnumber;
    rpsc = rpsarray[idx];
    distec = distance(pc, pe);
    assert(distec >= rpsc);
  }
...
///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// calctysharps()    Get the splitting point of an encroached subface.       //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////

void tetgenmesh::calctysharps(badface* bface, point newpoint)
{
  face encsub, sharpseg;
  point pa, pb, pc;
  REAL orthonorm[3];
  REAL rpca, rpcb, hlen;
  bool jumpa, jumpb;
  ...
  // abc has type SHARPS, and ab is sharp.
  pa = bface->forg;
  pb = bface->fdest;
  pc = bface->fapex;
  sspivot(bface->ss, sharpseg);
  ...
  // Get the protecting radius at a.
  encsub = bface->ss;
  assert(sorg(encsub) == pa);
  rpca = getrpc(&encsub);
  // Get the protecting radius at b.
  encsub = bface->ss;
  sesymself(encsub);
  assert(sorg(encsub) == pb);
  rpcb = getrpc(&encsub);
  // Get the half length of ab.
  hlen = 0.5 * distance(pa, pb);

  if (b->verbose > 2) {
    printf("    SHARPS: (%d, %d, %d) ra = %g, rb = %g, hl = %g.\n",
           pointmark(pa), pointmark(pb), pointmark(pc), rpca, rpcb, hlen);
  }

  // Decide the jump point, it may at a, or, b, or the mid point of ab.
  jumpa = jumpb = false;
  if (rpca >= rpcb) {
    if (rpca >= hlen) {
      jumpa = true;
  ...
  if (jumpa || jumpb) {
    edgeorthonormal(pa, pb, pc, orthonorm);
    if (jumpa) {
      for (i = 0; i < 3; i++) newpoint[i] = pa[i] + rpca * orthonorm[i];
      // a is the parent point of newpoint.
      setpoint2ppt(newpoint, pa);
    } else {
      for (i = 0; i < 3; i++) newpoint[i] = pb[i] + rpcb * orthonorm[i];
      // b is the parent point of newpoint.
      setpoint2ppt(newpoint, pb);
    }
  } else {
    // Split the segment at the middle.
    for (i = 0; i < 3; i++) newpoint[i] = 0.5 * (pa[i] + pb[i]);
    // No parent point is availabe.
    setpoint2ppt(newpoint, NULL);
  }
}
...
///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// calctyfsvsharps()    Get the splitting point of an encroached subface.    //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////

void tetgenmesh::calctyfsvsharps(badface* bface, point newpoint)
{
  face encsub;
  face sharpseg;
  point forg, fdest, fapex;
  ...
  if (b->verbose > 2) {
    printf("    FSVSHARPS: (%d, %d, %d) rpc = %g.\n", pointmark(forg),
           pointmark(bface->fdest), pointmark(bface->fapex), rpc);
  }

  // Get a sharp segment containing bface->forg.
  getorlsegments(&encsub, &sharpseg, NULL);
  assert(segmenttype(sharpseg) == SHARPSEGMENT);
  if (sorg(sharpseg) != forg) sesymself(sharpseg);
  assert(sorg(sharpseg) == forg);
  fdest = sdest(sharpseg);
  assert(fdest != bface->fdest && fdest != bface->fapex);
  // Get the jump direction.
  fapex = bface->fapex;
  edgeorthonormal(forg, fdest, fapex, orthonorm);
  
  // Set the newpoint.
  for (i = 0; i < 3; i++) newpoint[i] = forg[i] + rpc * orthonorm[i];
  // forg is the parent point of newpoint.
  setpoint2ppt(newpoint, forg);
}

///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// calctynavsharps()    Get the splitting point of an encroached subface.    //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////

void tetgenmesh::calctynavsharps(badface* bface, point newpoint)
{
  face encsub, checksh;
  face rseg, lseg;
  point pa, pb, pc;
  ...
  encsub = bface->ss;
  pa = bface->forg;
  assert(pointtype(pa) == NONACUTEVERTEX);

  // Get the two segment incident at a. 
  getorlsegments(&encsub, &rseg, &lseg);
  rsharp = (segmenttype(rseg) == SHARPSEGMENT);
  lsharp = (segmenttype(lseg) == SHARPSEGMENT);
  // At least one segment is sharp.
  assert(rsharp || lsharp);
  if (rsharp) {
    checksh = encsub;
    rpcr = getrpc(&checksh);
  }
  ...
  if (b->verbose > 2) {
    printf("    NAVSHARPS: (%d, %d, %d) rpc = %d.\n", pointmark(bface->forg),
           pointmark(bface->fdest), pointmark(bface->fapex), rpc);
  }

  edgeorthonormal(pb, pc, bface->fdest, orthonorm);
  for (i = 0; i < 3; i++) newpoint[i] = pa[i] + rpc * orthonorm[i];
  // a is the parent point of newpoint.
  setpoint2ppt(newpoint, pa);
}
...
///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// splitencsubs()    Split all the encroached subfaces.                      //
//                                                                           //
// Each encroached subface is repaired by splitting it - inserting a vertex  //
// at or near its circumcenter.  However, if the new vertex encroaches upon  //
// some subsegments, it is not inserted. Instead, all encroached subsegments //
// are split.  Newly inserted vertex may encroach upon other existing        //
// subsegments and subfaces; these are also repaired.                        //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////

void tetgenmesh::splitencsubs(queue* flipqueue, list* cavtetlist)
{
  badface *encsub;
  triface starttet;
  ...
  // Loop until the list 'encsubfaces' is empty.
  while (encsubfaces->items > 0) {
    // Look for an encsub from a nonempty queue. Remember the quenumber.
    encsub = (badface *) NULL;
    for (quenumber = 0; quenumber < 6; quenumber++) {
      encsub = subquefront[quenumber];
      if (encsub != (badface *) NULL) {
        // Remove the encsub from the queue.
        subquefront[quenumber] = encsub->nextface;
        if (subquefront[quenumber] != (badface *) NULL) {
          // The nextface exist! It becomes the first node.
          subquefront[quenumber]->prevface = (badface *) NULL;
        } else {
          // encsub is the last one. The queue is empty.
          subquetail[quenumber] = (badface *) NULL;
        }
        break;
      }
    }
    assert(encsub != (badface *) NULL);
    if (b->verbose > 2) {
      printf("    Dequeuing encsub x%lx (%d, %d, %d).\n",
             (unsigned long) encsub, pointmark(encsub->forg),
             pointmark(encsub->fdest), pointmark(encsub->fapex));
    }

    // Clear the pointer saved in encsub->ss. 
    splitsub = encsub->ss;
    setsub2badface(splitsub, NULL);
    ...
    enq = ((sorg(splitsub) == encsub->forg) &&
           (sdest(splitsub) == encsub->fdest) &&
           (sapex(splitsub) == encsub->fapex));
    if (enq) {
      // This subface is encroached, it should be split. Calculate a point
      //   at or near the circumcenter of this subface.
      newpoint = (point) points->alloc();
      // Initialize this field.
      setpoint2ppt(newpoint, NULL);
      // 'quenumber' implies the type of the encsub.
      ety = (enum encsubtype) quenumber;
      switch (ety) {
      case ACUTEVSHARPS:
        calctyacutevsharps(encsub, newpoint);
        break;
      case ACUTEV:
        calctyacutev(encsub, newpoint);
        break;
      case SHARPS:
        calctysharps(encsub, newpoint);
        break;
      case FSVSHARPS:
        calctyfsvsharps(encsub, newpoint);
        break;
      case NAVSHARPS:
        calctynavsharps(encsub, newpoint);
        break;
      default: // NAVNSHARPS
        calctynavnss(encsub, newpoint);
      }
      // Not interpolate its attributes.
      for (i = 0; i < in->numberofpointattributes; i++) newpoint[i + 3] = 0.0;
      // Set the unique index of the new point
      ptidx = (int) points->items - (in->firstnumber == 1 ? 0 : 1);
      setpointmark(newpoint, ptidx);
      // Set the type of the new point.
      ...
      // Find encroaching segments from cavtetlist.
      tallencsegs(newpoint, cavtetlist);
      // Clear the list for the next use.
      cavtetlist->clear();
      reject = (encsubsegs->items > 0);
      if (!reject) {
        ...
        assert(encsubsegs->items == 0);

      } else {
        // 'newpoint' is rejected.  Delete it.
        pointdealloc(newpoint);

        // Put 'encsub' back into the queue.
        if (b->verbose > 2) {
          printf("    Requeuing encsub x%lx.\n", (unsigned long) encsub);
        }
        setsub2badface(encsub->ss, encsub);
        // Queue elements are double-linked.
        encsub->prevface = (badface *) NULL;
        ...
        splitencsegs(flipqueue);
      }
    } else {
      // It is changed. It may still be encroached.
      checksub4encroach(&splitsub, NULL, true);
    }

    if (!enq || (enq && !reject)) {
      // Remove this entry from list.
      badfacedealloc(encsubfaces, encsub);
    }
  }
}
...
//                                                                           //
// In a conforming Delaunay tetrahedralization, the diametric circumballs of //
// all subsegments and subfaces are empty.                                   //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////
...
void tetgenmesh::enforcequality()
{
  ...
  if (!b->quiet) {
    printf("Conforming Delaunay.\n");
  }

  // Initialize 'flipqueue', cavtetlist.
  flipqueue = new queue(sizeof(badface));
  cavtetlist = new list(sizeof(triface), NULL, 256);
  // Initialize 'rpsarray', and 'apcarray'.
  rpsarray = new REAL[points->items];
  apcarray = new REAL[insegment * 2];
  
  // Initialize the pool of encroached subsegments.
  encsubsegs = new memorypool(sizeof(badface), BADSUBPERBLOCK, POINTER, 0);
  // Calculate the protecting spheres for all acute points.
  initializerpsarray();
  // Find all encroached segments.
  tallencsegs(NULL, NULL);
  if (encsubsegs->items > 0) {
    // Fix all encroached segments.
    splitencsegs(flipqueue);
  }

  // Initialize the pool of encroached subfaces.
  encsubfaces = new memorypool(sizeof(badface), BADSUBPERBLOCK, POINTER, 0);
  // Initialize the queues of encroached subfaces.
  for (i = 0; i < 5; i++) subquefront[i] = subquetail[i] = (badface *) NULL;
  // Marking sharp segments.
  marksharpsegments(89.0);
  // Calculate the protecting angles array.
  initializeapcarray();
  // Find all encroached subfaces.
  tallencsubs();
  if (encsubfaces->items > 0) {
    // Fix all encroached subface.
    splitencsubs(flipqueue, cavtetlist);
  }
...
//
// End of conforming Delaunay routines
//
...
// is initialized be the type INPUTVERTEX.  The bounding box ('xmin', 'xmax',//
// 'ymin', 'ymax', 'zmin', 'zmax') and the diameter ('longest') of the point //
// set are calculated in this routine as well.                               //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////
...
void tetgenmesh::transfernodes()
{
  ...
  coordindex = 0;
  attribindex = 0;
  pointnumber = in->firstnumber; // Index starts from 'in->firstnumber'.
  for (i = 0; i < in->numberofpoints; i++) {
    ...
    setpointtype(pointloop, INPUTVERTEX);
    ...
    pointnumber++; // The index of the next vertex.
  }
  ...
  if (longest == 0.0) {
    printf("Error:  Input point set is trivial.\n");
    exit(1);
  }
}
...
//                                                                           //
// Each point will be numbered, the first number is 'in->firstnumber'.       //
//                                                                           //
    ...
    if (bmark) {
      ...
      if (index < in->numberofpoints) {
        ...
        if (marker == 0) {
          ...
          marker = ((pointtype(pointloop) == INPUTVERTEX) ||
                    (pointtype(pointloop) == ACUTEVERTEX) ||
                    (pointtype(pointloop) == NONACUTEVERTEX) ||
                    (pointtype(pointloop) == FACETVERTEX)) 
                 ? 1 : 0;
        }
      } else if ((pointtype(pointloop) == ACUTEVERTEX) ||
                 (pointtype(pointloop) == NONACUTEVERTEX) ||
                 (pointtype(pointloop) == FREESEGVERTEX) ||
                 (pointtype(pointloop) == FREESUBVERTEX)) {
        // A booundary vertex has marker 1.
        marker = 1;
      } else {
...
///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// outmesh2gid()    Write mesh to a .gid.msh file which can be imported and  //
//                  rendered by Gid.                                         //
//                                                                           //
...
void tetgenmesh::outmesh2gid(char* gfilename)
{
  ...
  fprintf(outfile, "mesh dimension = 3 elemtype tetrahedra nnode = 4\n");
  ...
  // The boundary faces are followed after the tetrahedra.
  fprintf(outfile, "mesh dimension = 3 elemtype triangle nnode = 3\n");
  // There is no need to output coordinates here.
  fprintf(outfile, "coordinates\n");
  fprintf(outfile, "end coordinates\n");
  fprintf(outfile, "elements\n");
  ...
  while (faceloop.sh != (shellface *) NULL) {
    ...
    fprintf(outfile, "%5d  %5d %5d %5d", facenumber,
            pointmark(p1), pointmark(p2), pointmark(p3));
    if (bmark) {
      fprintf(outfile, "  %d", marker);
    }
  ...
  while (tetraloop.tet != (tetrahedron *) NULL) {
    ...
    for (tetraloop.loc = 0; tetraloop.loc < 4; tetraloop.loc++) {
      ...
      if (shouldbedelaunay) {
        ...
        if (sign > 0.0) {
          printf("  !! !! Non-locally Delaunay face.\n");
...
///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// ~mesh3d()    Deallocte memory occupied by an object of mesh3d.            //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////
//                                                                           //
// mesh3d()    Initialize an object of mesh3d.                               //
//                                                                           //
///////////////////////////////////////////////////////////////////////////////
...
tetgenmesh::tetgenmesh()
{
  ...
  encsubsegs = (memorypool *) NULL;
  encsubfaces = (memorypool *) NULL;
  badtetrahedrons = (memorypool *) NULL;
  illtetrahedrons = (memorypool *) NULL;
